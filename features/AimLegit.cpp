...
void AimLegit::Work(CUserCmd *usercmd)
{
	...
	if (g_Options.legit_enabled && (g_InputSystem->IsButtonDown(static_cast<ButtonCode_t>(g_Options.legit_aim_keybind1)) || g_InputSystem->IsButtonDown(static_cast<ButtonCode_t>(g_Options.legit_aim_keybind2))) && !g_LocalPlayer->m_hActiveWeapon().Get()->IsWeaponNonAim())
	{
		GetBestTarget();
		if (HasTarget())
			TargetRegion();
	...
	if (g_Options.legit_trigger)
		if (!g_Options.trigger_on_press || g_Options.trigger_on_press && g_InputSystem->IsButtonDown(g_Options.trigger_keybind))
			Triggerbot();
